...
def test_move_window_calls_movewindow(manager_mocks):
...
def test_execute_dispatches_drag(manager_mocks):
    ...
    mgr.execute(action)
    win32gui.MoveWindow.assert_called_once_with(1001, 110, 120, 400, 300, True)


def test_execute_scroll_up(manager_mocks):
    mgr, _, win32api = manager_mocks
    action = {"type": "SCROLL_UP", "window": {"hwnd": 1001}, "data": {}}
    mgr.execute(action)
    win32api.PostMessage.assert_called_once()


def test_win32_error_logged_not_raised(manager_mocks):
    """Exceptions in Win32 calls should be logged, not propagated."""
    mgr, win32gui, _ = manager_mocks
    win32gui.MoveWindow.side_effect = OSError("access denied")
